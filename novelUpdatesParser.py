...
class novelUpdatesParser:
    ...
    def parseTable(self, table):
        ...
        for row in rows:
            ...
            chapter = cols[-1]
            chapterLinks = chapter.find_all('a', href=True)
    ...
    def createBook(self):
        ...
        navCSS = epub.EpubItem(uid='style_nav', file_name='style/nav.css', media_type='test/css', content=style)
        ...
        bookPath = os.path.join(self.downloadPathSeries, "{}.epub".format(self.seriesTitle))
        epub.write_epub(bookPath, self.epub, {})
    ...
    def addChapter(self, chapter, html):
        chapterPath = "{}.xhtml".format(os.path.join(self.downloadPathSeries, chapter))
        chapter = epub.EpubHtml(title=chapter, file_name=chapterPath)
